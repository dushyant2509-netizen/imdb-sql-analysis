...
ranked_directors AS (
    SELECT *,
           ROW_NUMBER() OVER (PARTITION BY genre ORDER BY movie_count DESC) AS director_rank
...
AND r.avg_rating > 8
GROUP BY n.name
ORDER BY superhit_movie_count DESC,
...
SELECT
    ...
    AVG(DATEDIFF(date_published, prev_date)) AS avg_inter_movie_days,
